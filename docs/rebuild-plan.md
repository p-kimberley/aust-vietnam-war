...
| Vintage styles and tilesets | The vintage styles and tilesets live in two personal Mapbox accounts (`kimberleyp`, `gradata-systems`). MapLibre cannot load `mapbox://` resources, and Mapbox's terms do not allow using its hosted tiles from other libraries, so the vintage look is re-created as a style on the tile server: `deploy/map-styles/vintage/` is a TileServer GL package (a 1960s topographic sheet with contours, hillshade and land-cover tints, built from the `ww2` base) ready to import; until it is imported the app's Vintage basemap still points at `ww2`. Rotate the leaked Mapbox tokens regardless. |
